...
```{r echo=F}
table1 <- track %>% group_by(country2) %>% summarise(n = n()) %>% arrange(desc(n)) %>% filter(n > 15)
colnames(table1) <- c("Country", "Number of Athletes")

table1 %>% t() %>% kable() %>% kable_styling()
```
...
Level 1 Equation: $FinishingTime_{ij}=\beta_{0j} + \beta_{1j}(dist100_{ij})+\beta_{1}(cBMI_{ij}) +\beta_{2}(year1896_{ij})+\beta_{3}(sex_{ij})+\beta_{4}(sex_{ij}*dist100_{ij}) + \epsilon_{ij}$ \

Level 2 Equation: $\beta_{0j}=\beta_{00}+\beta_{01}(gdpPerCap_{ij})+u_{0j}$
...
Composite Equation: $FinishingTime_{ij}= \beta_{00}+\beta_{10}(dist100_{ij})+\beta_{01}(gdpPerCap_{ij})+\beta{11}(gdpPerCap_{j}*dist100_{ij}) +\beta_{1}(cBMI_{ij}) +\beta_{2}(year1896_{ij})+\beta_{3}(sex_{ij})+\beta_{4}(sex_{ij}*dist100_{ij})+u_{0j}+u_{1j}(dist100_{ij})+ \epsilon_{ij}$
