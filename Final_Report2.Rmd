---
title: "Predicting Success for Olympic Track Athletes with a Multilevel Model"
...
- `dist100:sexW`: After adjusting for the year of the race and the BMI of the athlete female athletes’ associated rate of increase in their finishing times per 100m increase of the race is 1.76 second higher than male athletes. 
    - After adjusting for the year of the race, the BMI of the athlete, and GDP per capita of the country the athlete is competing for, a female athlete is predicted to have a (13.54 + 0.02 $\times$ dist100) second slower finishing time than a male athlete. The plot below shows that the sex of the athlete has an effect on the association between distance of the race and finishing time.
...
for a country with a low GDP per capita is predicted to have a (17.11 + 0.01 $\times$ dist100) second 
...
```{r echo=F, fig.asp = 0.65}
track <- track %>% mutate(
  longdist = case_when(dist == 100 ~ "Short Distance",
                       dist == 200 ~ "Short Distance",
                       dist == 400 ~ "Short Distance",
                       dist == 800 ~ "Short Distance",
                       dist == 1500 ~ "Short Distance",
                       TRUE ~ "Long Distance") %>% as.factor())
