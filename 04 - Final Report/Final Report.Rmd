...
Still to do: 
  ...
  2. Tables of counts and summaries
```
...
```{r}
track %>% group_by(gdpPerCap_) %>% summarise(n = n()) %>% arrange(desc(n))
```
...
```{r}
ggplot(track, aes(y=timeSecs, x=c_BMI, color = country2)) +
