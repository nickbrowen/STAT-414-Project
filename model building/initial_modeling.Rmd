...
```{r}
ggplot(data = track, aes(x=dist, y = timeSecs, color = country2)) + geom_point(show.legend = F) + geom_jitter(height = .5, show.legend = F) + geom_smooth(method = "lm", se = F, show.legend = F)
```
...
summary(mod7)
```
Yes, gdp is significant, but it has a coefficient oof 0 and std error of 0. Looking closer and using lmer output, the coefficient is actually $1.882*10^{-12}$.  We will to use GDP in billions of dollars, to make this a more usefull variable.
...
Mod9 Residual Plots
```{r}
plot(residuals(mod9) ~ fitted.values(mod9)) 
abline(0,0)
```
...
```{r}
ranef_mod9 <- ranef(mod9)
ggplot(data=ranef_mod9, aes(ranef_mod9[,1])) + geom_histogram(bins = 15) + ggtitle("Random Intercepts for mod9")
```

```{r}
ggplot(data=ranef_mod9, aes(ranef_mod9[,2])) + geom_histogram(bins = 15) + ggtitle("Random Slopes for mod9")
```


Mod10 Residual Plots

```{r}
plot(residuals(mod10) ~ fitted.values(mod10)) 
abline(0,0)
```


Mod11 Residual Plots
```{r}
plot(residuals(mod11) ~ fitted.values(mod11)) 
abline(0,0)
```
...
```{r}
ranef_mod11 <- ranef(mod11)
ggplot(data=ranef_mod11, aes(ranef_mod11[,1])) + geom_histogram(bins = 15) + ggtitle("Random Intercepts for mod11")
```

```{r}
ggplot(data=ranef_mod11, aes(ranef_mod11[,2])) + geom_histogram(bins = 15) + ggtitle("Random Slopes for mod11")
```
